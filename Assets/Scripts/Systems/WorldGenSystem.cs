		...
		Entities
			...
			.ForEach((
				...
				for (int z = 0; z < worldData.CHUNK_SIZE; z++) {
					...
					float e1 = Ease((0.5f + noise.snoise(7.567f * n) / 2) + 0.02f, -8.57f);

					float e2 = Ease(((0.244f * noise.snoise(9.588f * n) * e1) + 1) / 2 + 0.041f, -3.36f) * 2 - 1;

					float e3 = Ease(((0.182f * noise.snoise(25.246f * n) * math.max(e1 - 0.433f, 0)) + 1) / 2, -1.57f) * 2 - 1;
